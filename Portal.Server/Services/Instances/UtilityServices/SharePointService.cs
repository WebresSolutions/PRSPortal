...
public class SharePointService : ISharePointService
{
    ...
    public SharePointService(SharePointOptions config, ILogger logger)
    {
        _options = config;
        _Logger = logger;
    ...
    public virtual async Task<string?> SaveFileAsync(Stream fileStream, string subFolder, string fileName, List<string> tag)
    {
        try
        {
            ...
            DriveItem uploadFolder = await CreateDirectoryStructure(subFolder)
        ...
        {
            _Logger.LogError(ex, "An Error occurred while saving a file to sharpoint.");
            throw;
        }
    }
    ...
    public virtual async Task<string?> CreateLinkToFile(string subFolder, string auditSubFolder)
    {
        try
        {
            ...
            DriveItem linkFolder = await CreateDirectoryStructure(subFolder)
                                   ?? throw new Exception($"Failed to get/create the folder for - {subFolder}");

            // The folder which the link is being added to.
            DriveItem uploadFolder = await CreateDirectoryStructure(auditSubFolder)
        ...
        string archiveDirectoryPath)
    {
        try
        {
            ...
            DriveItem archiveFolder = await CreateDirectoryStructure(archiveDirectoryPath)
    ...
    public virtual async Task<DriveItem?> CreateDirectoryStructure(string directory)
    {
        ...
        try
        {
            ...
            string[] safeFolderList = [.. directory.Split('/').Select(GetSafeFolderName)];
            ...
            try
            {
                DriveItem? existingItem = await graphClient.Drives[_options.DriveId].Root.ItemWithPath(directory).GetAsync();
            ...
            {
                _Logger.LogInformation(ex, $"File path: {directory} does not exist in sharepoint");
            }
            finally
            {
                ...
                foreach (string subFolderName in safeFolderList)
                {
                    ...
                    RequestInformation createFolderRequest = graphClient.Drives[_options.DriveId].Root.ItemWithPath(previousPath).Children
                    ...
                    batchRequestContent.AddBatchRequestStep(new BatchRequestStep(
                        ...
                        [.. dependency.Select(id => id.ToString())])); // Convert int to string for dependencies
        ...
        {
            _Logger.LogError(ex, "Failed to create directory in sharepoint");
            throw;
        }
        ...
        return await graphClient.Drives[_options.DriveId].Root.ItemWithPath(directory).GetAsync();
    }
