...
public partial class JobNotes
{
    ...
    private void SearchNotes(string search)
    {
        Notes = [.. NotesCopy.Where(x => x.Content.Contains(search))];
    }
    ...
    private async Task DeleteNoteAsync(JobNoteDto note)
    {
        ...
        Result<List<JobNoteDto>> res = await _apiService.SaveJobNote(note);
        if (res.IsSuccess)
            Notes = res.Value ?? [];
    }
    ...
    private async Task ChangeTabs(TabTypeEnum Tab)
    {
        try
        {
            ...
            List<JobNoteDto> Handlenotes(Result<List<JobNoteDto>> notes)
            {
